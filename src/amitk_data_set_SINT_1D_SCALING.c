...
void amitk_data_set_SINT_1D_SCALING_calc_slice_min_max(AmitkDataSet * data_set,
  ...
  temp = AMITK_DATA_SET_SINT_1D_SCALING_CONTENT(data_set, i);
  ...
  for (i.t = 0; (i.t < data_set_dim.t) && continue_work; i.t++) {
	...
	for (i.y = 0; i.y < data_set_dim.y; i.y++) 
	  for (i.x = 0; i.x < data_set_dim.x; i.x++) {
	    j.x = scale*(AMITK_DATA_SET_SINT_1D_SCALING_CONTENT(data_set,i)-amitk_data_set_get_global_min(data_set));
	...
	for (z = 0; z < ceil(z_steps); z++) {
	  ...
	  for (i_voxel.y = start.y,k=0; i_voxel.y <= end.y; i_voxel.y++) {
		...
		if (amitk_raw_data_includes_voxel(data_set->raw_data, box_voxel[l]))
		  box_value[l] = AMITK_DATA_SET_SINT_1D_SCALING_CONTENT(data_set, box_voxel[l]);
		else {
		  box_value[l] = NAN;
		  empties = TRUE;
		}
	...
	case AMITK_RENDERING_MPR:
	  ...
	  for (z = 0; z < ceil(z_steps); z++) { 
		...
		if (amitk_raw_data_includes_voxel(data_set->raw_data,ds_voxel)) {
		  intermediate_data[k] +=
		    weight*AMITK_DATA_SET_SINT_1D_SCALING_CONTENT(data_set,ds_voxel);
		  weights[k] += weight;
		}
	  ...
	  for (z = 0; z < ceil(z_steps); z++) { 
		...
		for (i_voxel.x = start.x; i_voxel.x <= end.x; i_voxel.x++,k++) {
		  ...
		  if (!amitk_raw_data_includes_voxel(data_set->raw_data,ds_voxel)) 
		    ...
		    intermediate_data[k] =
		      AMITK_DATA_SET_SINT_1D_SCALING_CONTENT(data_set,ds_voxel);
		  POINT_ADD(ds_point, stride[AMITK_AXIS_X], ds_point); 
		} /* x */
		POINT_ADD(last[AMITK_AXIS_Y], stride[AMITK_AXIS_Y], ds_point);
	      } /* y */
