...
#define __AMITK_ROI_CYLINDER_H__
...
#define ROI_TYPE_CYLINDER

#if defined(ROI_TYPE_ELLIPSOID) || defined(ROI_TYPE_CYLINDER) || defined(ROI_TYPE_BOX)
GSList * amitk_roi_CYLINDER_get_intersection_line(const AmitkRoi * roi, 
...
AmitkDataSet * amitk_roi_CYLINDER_get_intersection_slice(const AmitkRoi * roi,
								     const AmitkVolume * canvas_slice,
								     const amide_real_t pixel_dim
...
void amitk_roi_CYLINDER_set_isocontour(AmitkRoi * roi, 
						   AmitkDataSet * ds, 
						   AmitkVoxel iso_vp,
						   amide_data_t iso_min_value,
						   amide_data_t iso_max_value,
						   AmitkRoiIsocontourRange iso_range);
void amitk_roi_CYLINDER_manipulate_area(AmitkRoi * roi, gboolean erase, AmitkVoxel voxel, gint area_size);
void amitk_roi_CYLINDER_calc_center_of_mass(AmitkRoi * roi);
#endif

void amitk_roi_CYLINDER_calculate_on_data_set_fast(const AmitkRoi * roi,  
							       ...
							       void (*calculation)(),
							       gpointer data);
void amitk_roi_CYLINDER_calculate_on_data_set_accurate(const AmitkRoi * roi,  
								   const AmitkDataSet * ds, 
								   const guint frame,
								   const guint gate,
								   const gboolean inverse,
...
#undef ROI_TYPE_CYLINDER

#endif /* __AMITK_ROI_CYLINDER_H__ */
