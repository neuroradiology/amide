...
AmitkRawData * amitk_raw_data_DOUBLE_0D_SCALING_init(amitk_format_DOUBLE_t init_value) {
  ...
  temp_amitk_raw_data = amitk_raw_data_new_with_data(AMITK_FORMAT_DOUBLE, one_voxel);
  g_return_val_if_fail(temp_amitk_raw_data != NULL, NULL);
  i = zero_voxel;

  (*AMITK_RAW_DATA_DOUBLE_0D_SCALING_POINTER(temp_amitk_raw_data, i)) = init_value;
  ...
  for (i.t = 0; i.t < amitk_raw_data->dim.t; i.t++)
	...
	for (i.y = 0; i.y < amitk_raw_data->dim.y; i.y++) 
	  for (i.x = 0; i.x < amitk_raw_data->dim.x; i.x++) 
	    AMITK_RAW_DATA_DOUBLE_SET_CONTENT(amitk_raw_data,i)=init_value;
